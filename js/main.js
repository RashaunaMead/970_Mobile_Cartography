...
function initialize(){
  ...
  queue()
    ...
    .await(callback);
  //setMap(map); // sends to responsive.js this allows map elements to be responsive
}
...
function callback(error, routes, PointsofInterest, POI){
  ...
  function updateRoute(){
    console.log(viewed);
    // show the route to next site after the previous site was viewed (SWITCH TO SITEID)
    for(var i=0; i<viewed.length-1; i++){
      if(viewed[i] && !viewed[i+1]){
        L.geoJson(routes.features[i+1], routeStyle).addTo(map); //visited style route underlays highlight
      }
    }
  }
  ...
  $(document).on('close.fndtn.reveal', '[data-reveal]', function (){ triggerIconBubble() });
